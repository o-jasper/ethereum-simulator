...
module.exports = function(settings) {
  ...
  function templateFile(tfile) { //Decided to keep the .hbs to still be specified.
    return ("./templates/" +
            (settings.templateDir ==null ? "default" : settings.templateDir) +
            "/"  + tfile);
  }
  function templateFileCompiled(tfile) {
    var cfile = templateFile(tfile);
    var str = fs.readFileSync(cfile).toString();
    return Handlebars.compile(str);
  }

  function prepareTemplates() {
    var appTemplates = {
      contract       : templateFileCompiled('contract.hbs'),
      transaction    : templateFileCompiled('transaction.hbs'),
      wallet         : templateFileCompiled('wallet.hbs'),
      new_transaction: templateFileCompiled('new_transaction.hbs'),
      new_contract   : templateFileCompiled('new_contract.hbs'),
      new_wallet     : templateFileCompiled('wallet.hbs'),
      about          : templateFileCompiled('about.hbs')
    }
    return appTemplates
  }
